...
public sealed class CalibrationProfilePersistenceTests
{
    ...
    [Fact]
    public void ExplicitMigrationUsesCallerIdentityAndRetainsReversibleOriginal()
    {
        var legacy = LegacyProfile();
        ...
        Assert.Equal(ControllerRuntimeIdentities.MetaLinkLibOvr, result.MigratedProfile.ControllerRuntime);
        Assert.Equal("META-RUNTIME-IDENTITY", result.MigratedProfile.ControllerIdentity);
    ...
    [Fact]
    public void OptionalControllerIdentityIsOmittedAndNullableQualityFieldsRemainExplicit()
    {
        var profile = Profile(
        ...
        var loaded = CalibrationProfileJson.DeserializeProfile(json);
        Assert.Null(loaded.ControllerIdentity);
