...
This tracked checklist contains 64 live acceptance items: the 64 lines below
that begin with `- [ ]`. All 64 remain unchecked. The repository does not
...
The `Build internal drivers` workflow first detects the pinned
`src/Ltb.OpenVr/ThirdParty/OpenVR/openvr_driver.h`. When the header is present,
the Windows job automatically configures with `LTB_BUILD_OPENVR_DRIVER=ON`,
builds and tests the native targets, verifies the staged driver root and build
identity, checks the produced DLL's regular and delay-load PE imports, and
uploads that staged driver. The package job consumes the exact uploaded
artifact, verifies the clean-source manifest and archived DLL hash, and uploads
the portable ZIP and checksum. When the pinned header is absent, the workflow
continues managed and portable native-core validation but explicitly defers
the full Windows driver and package gates instead of downloading or
substituting a header.
...
only. A successful full-driver workflow proves source, build, staging, import,
and package integrity for its commit. It does not prove SteamVR registration
or load, staged-versus-loaded identity, Meta runtime ABI behavior, GUI behavior,
or connected hardware. Those claims remain gated by the unchecked items below.
