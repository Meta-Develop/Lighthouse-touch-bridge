...
The tracked internal-driver workflow first detects
`src/Ltb.OpenVr/ThirdParty/OpenVR/openvr_driver.h` in the checked-out source.
When the pinned header is present, the Windows CMake configure automatically
sets `LTB_BUILD_OPENVR_DRIVER=ON`, builds and stages the complete driver,
checks its PE imports, uploads that exact artifact, and passes it into the
portable-package build. When the header is absent, only the full driver and
package path is explicitly deferred; managed and portable native validation
still run, and the workflow never downloads or substitutes an untracked
header.
...
used as import evidence. Even a successful complete workflow remains
build/package evidence rather than on-device SteamVR, Meta, or hardware
acceptance.
...
The tracked checklist contains 64 live acceptance items: the 64 lines that
begin with `- [ ]`. All 64 remain unchecked, and the repository does not
