...
and the IPC v1 battery field is exactly zero. IPC v1 rejects the
battery-present flag and any nonzero battery value. A later battery source
requires a versioned protocol and capability change.
...
| Reserved battery field | battery-present flag clear and battery value exactly zero |
...
out-of-range analog values, any battery-present flag or nonzero battery value,
degenerate or non-unit quaternions, inconsistent validity flags, replayed
sequences, and per-stream regressing timestamps. A rejected packet shall never
partially update either hand.
...
logging. Avalonia 11 is the selected desktop UI framework, isolated in
`Ltb.Gui`; the decision and dependency boundary are recorded in
...
The repository's internal-driver workflow first detects the documented pinned
`openvr_driver.h` in the checked-out source. When the header is present, the
workflow automatically configures the Windows build with
`-DLTB_BUILD_OPENVR_DRIVER=ON`, verifies the staged driver layout and PE import
boundary, uploads the staged driver, and builds and verifies the portable
package. When the header is absent, only the full Windows driver and package
coverage is deferred; managed tests and the portable native core remain
required. Neither path is live runtime or hardware acceptance.

Windows CI where safe, or a controlled Windows test host, shall additionally
prove:
