...
internal sealed class InternalDriverSession : IInternalDriverSession
{
    ...
    private async Task RunCoreAsync(CancellationToken cancellationToken)
    {
        ...
        finally
        {
            ...
            var finalSnapshot = faultSnapshot ??
                (CurrentSnapshot.State == InternalDriverSessionState.Faulted
                    ? CurrentSnapshot
                    : CreateStateSnapshot(
                        InternalDriverSessionState.Stopped,
    ...
    {
        TimeSpan? age = null;
        if (health.LastSuccessfulSendNanoseconds is { } last)
        {
            var now = _runtime.GetMonotonicNanoseconds();
            age = TimeSpan.FromTicks((long)Math.Min(long.MaxValue, (now >= last ? now - last : 0) / 100UL));
        }
