...
| 120 | 4 | stick X `[-1,1]` |
| 124 | 4 | stick Y `[-1,1]` |
| 128 | 4 | battery, exactly zero in IPC v1 |

The heartbeat ends after offset 47 and has payload length 32. The hand state
ends after offset 131 and has payload length 116. Accepted flag bits are
connected, orientation valid, position valid, linear velocity valid, angular
velocity valid, inputs valid, and tracked (bits 0 through 5 and 7). Bit 6 is
reserved for a future battery-present contract but is rejected in IPC v1, as
is any nonzero battery field. Unknown flags and input bits are rejected.
...
exactly zero. A disconnected state must clear all tracking, pose-validity,
velocity-validity, and input-validity flags.
...
## OpenVR source boundary

The Windows shell includes the unmodified `openvr_driver.h` from the pinned
OpenVR SDK 2.15.6 commit recorded in
[`src/Ltb.OpenVr/ThirdParty/OpenVR/README.md`](../../src/Ltb.OpenVr/ThirdParty/OpenVR/README.md).
That record contains the official source URL, Git blob, SHA-256, and Valve BSD
notice path. A server driver consumes the header interfaces and does not link
the OpenVR client `openvr_api.dll`.
...
`.github/workflows/build-internal-drivers.yml` detects the pinned
`src/Ltb.OpenVr/ThirdParty/OpenVR/openvr_driver.h` before configuring the
Windows build. When the header is present, the workflow automatically enables
`LTB_BUILD_OPENVR_DRIVER`, builds the x64 shell and portable tests, verifies the
staged file set and deterministic build identity, checks the produced DLL's PE
imports, uploads the staged driver, and packages that exact artifact. When the
header is absent, portable managed and native-core validation still runs, but
the full Windows driver and package gates are explicitly deferred; the
workflow never downloads or substitutes an untracked header.

These automated gates establish source, build, staging, import, and package
integrity. They do not prove that SteamVR loads the driver, that the staged and
loaded identities match on a user system, that Meta Horizon Link supplies live
input, or that connected controllers and trackers behave correctly. Keep those
items unchecked until the controlled Windows evidence in
[`docs/windows-internal-driver-verification.md`](../../docs/windows-internal-driver-verification.md)
